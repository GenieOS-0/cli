 ...
 *   genie links list|get|create|update|analytics|utm-suggestions
...
async function cmdLinks(args: ParsedArgs): Promise<void> {
  ...
  if (sub === 'get') {
    const linkId = flag(args, 'link-id') ?? args.positionals.shift();
    if (!linkId) fail('genie links get <linkId>');
    info(asJson(await gos.links.get(linkId)));
    return;
  }
  if (sub === 'update') {
    const linkId = flag(args, 'link-id') ?? args.positionals.shift();
    if (!linkId) {
      fail(
        'genie links update <linkId> [--url=…] [--label=…] [--tags=…] ' +
          '[--password=…] [--clear-password] [--expires-at=…] [--clear-expires-at] ' +
          '[--scheduled-go-live-at=…] [--clear-scheduled-go-live-at]',
      );
    }
    const tagsRaw = flag(args, 'tags');
    const tags = tagsRaw
    ...
    info(
      asJson(
        await gos.links.update(linkId, {
          ...(flag(args, 'url') ? { destinationUrl: flag(args, 'url') } : {}),
          ...(flag(args, 'label') ? { label: flag(args, 'label') } : {}),
          ...(bool(args, 'clear-label') ? { clearLabel: true } : {}),
          ...(tags?.length ? { tags } : {}),
          ...(flag(args, 'password') ? { password: flag(args, 'password') } : {}),
          ...(bool(args, 'clear-password') ? { clearPassword: true } : {}),
          ...(flag(args, 'expires-at') ? { expiresAt: flag(args, 'expires-at') } : {}),
          ...(bool(args, 'clear-expires-at') ? { clearExpiresAt: true } : {}),
          ...(flag(args, 'scheduled-go-live-at')
            ? { scheduledGoLiveAt: flag(args, 'scheduled-go-live-at') }
            : {}),
          ...(bool(args, 'clear-scheduled-go-live-at')
            ? { clearScheduledGoLiveAt: true }
            : {}),
        }),
      ),
    );
  ...
  if (sub === 'analytics') {
    const cardKey = (flag(args, 'card-key') ?? args.positionals.shift() ?? 'headline') as Parameters<
      typeof gos.links.analytics
    >[0]['cardKey'];
    const linkId = flag(args, 'link-id');
    const daysRaw = flag(args, 'days');
    info(
      asJson(
        await gos.links.analytics({
          cardKey,
          ...(linkId ? { linkId } : {}),
          ...(daysRaw ? { days: Number(daysRaw) } : {}),
          ...(bool(args, 'force-refresh') ? { forceRefresh: true } : {}),
        }),
      ),
    );
    return;
  ...
  fail(
    `Unknown subcommand: links ${sub}\n` +
      '  Available: list, utm-suggestions, create, get, update, analytics',
  );
}

async function cmdQr(args: ParsedArgs): Promise<void> {
  const sub = args.positionals.shift();
  const gos = makeClient();
  if (sub === 'create') {
    const linkId = flag(args, 'link-id');
    ...
    if (!linkId && !payload) {
      fail(
        'genie qr create --link-id <linkId> | --payload <text> [--label=…] [--tags=…]',
      );
    }
    const tagsRaw = flag(args, 'tags');
    const tags = tagsRaw
    ...
    info(
      asJson(
        await gos.qr.create({
          encodes: linkId
            ? { kind: 'shortLink', linkId }
            : { kind: 'static', payload: payload! },
          ...(flag(args, 'label') ? { label: flag(args, 'label') } : {}),
          ...(tags?.length ? { tags } : {}),
        }),
      ),
  ...
  if (sub === 'render') {
    const qrId = flag(args, 'qr-id') ?? args.positionals.shift();
    if (!qrId) {
      fail(
        'genie qr render <qrId> [--format=svg|png|png-print|webp|pdf] [--size=…] [--save-to-assets]',
      );
    }
    const format = flag(args, 'format') as
      | 'svg'
      | 'png'
      | 'png-print'
      | 'webp'
    ...
    info(
      asJson(
        await gos.qr.render(qrId, {
          ...(format ? { format } : {}),
          ...(sizeRaw ? { size: Number(sizeRaw) } : {}),
          ...(bool(args, 'save-to-assets') ? { saveToAssets: true } : {}),
        }),
      ),
    );
    return;
  }
  fail(`Unknown subcommand: qr ${sub}\n  Available: create, render`);
}
...
COMMANDS
  ...
  links get <linkId>                                                   Read one short link
  links update <linkId> [--url=…] [--password=…] [--expires-at=…] …    Free edits to a short link
  links analytics [--card-key=headline] [--link-id=…] [--days=…]       Click analytics
  qr create --link-id=… | --payload=… [--label=…]                      Create a QR design (1 credit)
  qr render <qrId> [--format=png] [--size=…] [--save-to-assets]         Render a QR to bytes
